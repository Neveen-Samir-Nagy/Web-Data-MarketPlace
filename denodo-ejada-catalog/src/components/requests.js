...
const Row = ({ request, wsname, index, viewName, handleNewStatus }) => {
    ...
    <TableBody sx={{ width: "100%" }}>
      <TableRow key={request.id}>
        ...
        {secureLocalStorage.getItem("manager") === "yes" ? (
          <Button
            disabled={request.status != "UNDERAPPROVAL"}
...
export default function CollapsibleTable() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:3000/all-requests`).then((response) => {
      var nreq = response.data.reduce((requests, curRequest) => {
  ...
  const handleGetView = (item) => {
    console.log()
    axios
      .get(
        "http://localhost:3000/ws-details/" +
          request[item][0].database_name +
          "/" +
          item +
          ""
      )
      .then((res) => {
        console.log(Object.keys(JSON.parse(res.data[0]).schema)[0]);

        setViewName(Object.keys(JSON.parse(res.data[0]).schema)[0]);
      })
